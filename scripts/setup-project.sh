...
REPO_URL="https://github.com/pedromshin/create-turbo-stack"
...
echo -e "\n${BLUE}Next steps:${NC}"
echo -e "1. Generate schema-based migrations: ${GREEN}pnpm db:migration:generate${NC} (automatically generates SQL from schema changes)"
echo -e "2. Create custom SQL migrations if needed: ${GREEN}pnpm db:migration:generate:custom your_migration_name${NC} (for triggers, functions, etc.)"
echo -e "3. Apply migrations locally: ${GREEN}pnpm db:migrate:local${NC} (test in local environment)"
echo -e "4. Deploy to production: ${GREEN}git push origin main${NC} (automatic deployment) or ${GREEN}pnpm db:migrate:prod${NC} (manual migration)"
echo -e "5. Start development with: ${GREEN}pnpm dev${NC}"